...
if ($_SERVER["REQUEST_METHOD"] != "POST") {
    header("Location: ../View/user_views/createAccount.php");
    exit();
}
    ...
    if ($_FILES["profile_photo"]["error"] != UPLOAD_ERR_OK) {
        ...
        if (empty($errors["profile_photo"])) {
            ...
            if (move_uploaded_file($fileTmp, $filePath)) {
                $profilePhoto = "storage/user/profile/" . $newFileName; // lowercase, matches saved dir
            } else {
...
if (!empty($errors)) {
    ...
    $_SESSION["old"] = [
        ...
        "gender" => $gender, "account_type" => $accountType, "location" => $location,
        "looking_for" => $lookingFor, "min_budget" => $minBudget, "max_budget" => $maxBudget,
        "pref_location" => $prefLocation, "move_in_date" => $moveInDate, "occupation" => $occupation
    ];
    header("Location: ../View/user_views/createAccount.php");
    exit();
}

$userTypeId = ($accountType == "owner") ? 2 : 3;
